"""
Flask server — OpenEnv-compliant API for SecureReviewAI.

Endpoints:
    GET|POST /          → health check
    GET|POST /reset     → reset environment, return first observation
    POST     /step      → submit action, receive reward + next observation
    GET|POST /state     → current session state
    GET      /tasks     → enumerate all tasks (for validator)
"""
